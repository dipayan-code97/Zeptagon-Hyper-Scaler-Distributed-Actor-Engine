...
Zeptagon provides the capacity of handling **10 million messages in under 1 second**. as a Distributed Actor Model Engine

## What is the actor model?

The Zeptagon Actor Model is a computational model used to build highly concurrent and distributed systems. It was introduced by
Carl Hewitt in 1973 as a way to handle complex systems in a more scalable and fault-tolerant manner.

In the Actor Model, the basic building block is an actor, sometimes referred to as a receiver in Zeptagon, 
which is an independent unit of computation that communicates with other actors by exchanging messages. 
Each actor has its own state and behavior, and can only communicate with other actors by sending messages.
This message-passing paradigm allows for a highly decentralized and fault-tolerant system, 
as actors can continue to operate independently even if other actors fail or become unavailable.
...
```
go get github.com/dipayan-code97/Zeptagon-Hyper-Scalar-Distributed-Actor-Engine...
```

>Zeptagon requires Golang version `1.21`
...
```
This creates a new engine. The engine is the core of Zeptagon. It's responsible for spawning actors, sending messages
and handling the lifecycle of actors. If Zeptagon fails to create the engine it'll return an error. For development 
...
function `newHelloer`. Ids must be unique. It will return a pointer to a PID. A PID is a process identifier.
It's a unique identifier for the actor. Most of
...
This will send a message to the actor. Zeptagon will route the message to the correct actor. The actor will then print
a message to the console.

The **[examples](examples/)** folder is the best place to learn and
explore Zeptagon further.
...
```go
pid := engine.Spawn(newCustomNameResponder("dipayan"), "name-responder")
```
...
This works the same as local actors but "over the wire". Zeptagon supports serialization with protobuf.
...
stream in order to receive events. As a bare minimum, you'll want  to handle `DeadLetterEvent`. If Zeptagon fails to 
...
Zeptagon has some built in logging. It will use the default logger from the `log/slog` package. You can configure the
